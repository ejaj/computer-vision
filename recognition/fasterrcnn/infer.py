...
def get_iou(det, gt):
    ...
    Args:
        det (tuple): Detected bounding box (x1, y1, x2, y2).
        gt (tuple): Ground truth bounding box (x1, y1, x2, y2).

    Returns:
        float: IoU value (0.0 to 1.0), where 1.0 is perfect overlap and 0.0 is no overlap.
    """
...
def compute_map(det_boxes, gt_boxes, iou_threshold=0.5, method='area'):
    ...
    Args:
        det_boxes (list of dict): Detected boxes per image, mapping class labels to 
                                  [x1, y1, x2, y2, score].
        gt_boxes (list of dict): Ground truth boxes per image, mapping class labels to 
                                 [x1, y1, x2, y2].
        iou_threshold (float): IoU threshold to classify a detection as a true positive. 
                               Default is 0.5.
        method (str): Method for AP calculation ('area' or 'interp'). Default is 'area'.

    Returns:
        tuple:
            - float: Mean Average Precision (mAP).
            - dict: AP for each class.

    Raises:
        ValueError: If `method` is not 'area' or 'interp'.
    """
...
def load_model_and_dataset(config):
    """
    Load a Faster R-CNN model and test dataset based on the provided configuration.

    Args:
        config (dict): Configuration dictionary with keys:
            - 'dataset_params': Paths for images and annotations.
            - 'model_params': Parameters for the model.
            - 'train_params': Training-related parameters, including seed and checkpoint info.

    Returns:
        tuple: 
            - Faster R-CNN model loaded with weights.
            - VOCDataset object for testing.
    ...
    voc = VOCDataset('test', img_dir=dataset_config['img_test_path'], ann_dir=dataset_config['ann_test_path'])
...
def infer(config):
    """
    Perform inference using a Faster R-CNN model and save visualizations of ground truth 
    and predicted bounding boxes.

    Args:
        config (dict): Configuration dictionary for model and dataset.

    Outputs:
        Saves annotated images with ground truth (`output_frcnn_gt_*.png`) and predictions 
        (`output_frcnn_*.jpg`) in the 'samples' directory.
    """
    if not os.path.exists('samples'):
        os.mkdir('samples')
    faster_rcnn_model, voc, test_dataset = load_model_and_dataset(config)
...
def evaluate_map(config):
    """
    Evaluate the Faster R-CNN model using mean Average Precision (mAP) and class-wise AP.

    Args:
        config (dict): Configuration dictionary for model and dataset.

    Outputs:
        Prints class-wise Average Precision (AP) and Mean Average Precision (mAP).
    """
    faster_rcnn_model, voc, test_dataset = load_model_and_dataset(config)
    gts = []
    preds = []
