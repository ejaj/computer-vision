...
from recognition.yolo_v1.visualization_utils import plot_image_with_yolo_targets_and_ground_truth


def load_images_and_anns(im_sets, label2idx, ann_fname, split):
    """
    Load image information and annotations from a dataset.

    Args:
        im_sets (list): Paths to image sets.
        label2idx (dict): Mapping from class names to indices.
        ann_fname (str): File with image names (e.g., trainval.txt/test.txt).
        split (str): Dataset split (train/test).

    Returns:
        list: Image information and associated ground truth detections.
    """
    im_infos = []
...
if __name__ == "__main__":
    # Define paths to VOC dataset directories
    voc_datasets = [
        "/home/kazi/Works/Dtu/computer-vision/recognition/fasterrcnn/data/VOCtrainval_06-Nov-2007/VOCdevkit/VOC2007",
    ...
    # Access and print a sample
    sample_index = 0
    image_tensor, targets, filename = dataset[sample_index]

    print(f"Image filename: {filename}")
    print(f"Image tensor shape: {image_tensor.shape}")
    print(f"Bounding boxes: {targets['bboxes']}")
    print(f"YOLO targets: {targets['yolo_targets'].shape}")
    print(f"Labels: {targets['labels']}")

    for i in range(3):
        image_tensor, targets, filename = dataset[i]
        print(f"Plotting for image: {filename}")
        plot_image_with_yolo_targets_and_ground_truth(
            image_tensor,
